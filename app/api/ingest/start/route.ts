...
export async function POST(request: NextRequest): Promise<NextResponse<StartIngestionResponse | StartIngestionError>> {
  ...
  try {
    ...
    if (dbError) {
      console.error('[ingest/start] Failed to create ingestion record:', dbError);
      // Clean up KV job if DB insert fails
      const { deleteJob } = await import('@/lib/jobs/queue');
      await deleteJob(jobId);
      throw new Error(`Failed to create ingestion record: ${dbError.message}`);
    }

    // Create processing steps in database
    const steps = ['script_preprocess', 'core_extraction', 'character_bible', 'visuals', 'market_adaptation', 'package_assembly', 'final_package'];
    const stepRows = steps.map((name) => ({
      ingestion_id: jobId,
      name,
      status: 'queued'
    }));
    ...
    if (stepsError) {
      console.error('[ingest/start] Failed to create ingestion steps:', stepsError);
      // Clean up ingestion record and KV job
      await supabase.from('ingestions').delete().eq('id', jobId);
      const { deleteJob } = await import('@/lib/jobs/queue');
      await deleteJob(jobId);
      throw new Error(`Failed to create ingestion steps: ${stepsError.message}`);
    }

    console.log(`[ingest/start] Created ingestion record with ${steps.length} steps`);
