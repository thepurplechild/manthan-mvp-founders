...
async function processPdfParsing(task: PdfTask) {
  try {
    // Send initial progress
    parentPort?.postMessage({
      type: 'progress', 
      data: { taskId: task.taskId, progress: 20, step: 'Loading PDF document' }
    });
    ...
    parentPort?.postMessage({
      type: 'progress',
      data: { taskId: task.taskId, progress: 40, step: 'Parsing PDF content' }
    });
    ...
    parentPort?.postMessage({
      type: 'progress',
      data: { taskId: task.taskId, progress: 80, step: `Extracted text from ${pdfData.numpages} pages` }
    });
...
async function processPdfOcr(task: PdfTask) {
  try {
    parentPort?.postMessage({
      type: 'progress',
      data: { taskId: task.taskId, progress: 20, step: 'Initializing OCR processing' }
    });
    ...
    parentPort?.postMessage({
      type: 'progress',
      data: { taskId: task.taskId, progress: 30, step: 'Loading PDF for OCR' }
    });
    ...
    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
      ...
      parentPort?.postMessage({
        ...
        data: { 
          ...
          step: `OCR processing page ${pageNum}/${totalPages}`
        }
      });
    ...
    parentPort?.postMessage({
      type: 'progress',
      data: { taskId: task.taskId, progress: 95, step: 'OCR processing complete' }
    });
